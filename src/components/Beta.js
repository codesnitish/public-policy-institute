...
function makePageTexture(pageIdx, isFront) {
    const cw = 512;
    const ch = 700;
    ...
    if (item.isCover) {
        if (isFront) {
            ...
            if (titleWords[1])
                g.fillText(titleWords[1], cw / 2, 270);
            g.fillStyle = '#ffffff';
...
function curlPage(pivot, angle, isCover) {
...
export default function BookSimulation() {
    ...
    useEffect(() => {
        ...
        const backCoverMesh = new THREE.Mesh(new THREE.BoxGeometry(PW + 0.1, PH + 0.15, 0.06), new THREE.MeshStandardMaterial({ color: 0xb685cc, roughness: 0.9 }));
        ...
        // Binding Spine
        const spine = new THREE.Mesh(new THREE.BoxGeometry(0.15, PH + 0.15, N * THICK + 0.1), new THREE.MeshStandardMaterial({ color: 0x9f6ab5, roughness: 0.95 }) // Harmonized light purple spine
        );
        ...
        for (let i = 0; i < N; i++) {
            ...
            const frontMesh = new THREE.Mesh(geo, new THREE.MeshBasicMaterial({
                map: frontTextures[i],
                color: 0xffffff,
            ...
            const backMesh = new THREE.Mesh(geo.clone(), new THREE.MeshBasicMaterial({
                map: backTextures[i],
                color: 0xffffff,
                side: THREE.BackSide,
            }));
        ...
        function startDrag(cx) {
            ...
            if (s.dragPage >= 0)
                s.dragA0 = s.angles[s.dragPage];
        }
        function moveDrag(cx) {
            if (!s.dragging || s.dragPage < 0)
                return;
    ...
    return (_jsxs(Box, { sx: { background: '#f0f4f8', minHeight: '100vh', overflow: 'hidden', userSelect: 'none', position: 'relative' }, children: [_jsx(Box, { ref: containerRef, sx: {
                    width: '100%',
                    height: { xs: 'min(90vh, 820px)', sm: 'min(80vh, 750px)' },
                    minHeight: { xs: 520, sm: 480 },
                    cursor: 'grab',
                    '&:active': { cursor: 'grabbing' },
                } }), _jsx(Typography, { sx: { position: 'fixed', top: 32, left: '50%', transform: 'translateX(-50%)', color: '#1f2937', fontFamily: 'Georgia, serif', fontSize: 18, fontWeight: 600, letterSpacing: 2, pointerEvents: 'none' }, children: BOOK_TITLE }), _jsx(Typography, { sx: { position: 'fixed', bottom: 90, left: '50%', transform: 'translateX(-50%)', color: '#64748b', fontFamily: 'Inter, sans-serif', fontSize: 12, pointerEvents: 'none', transition: 'opacity 1.5s', opacity: hintOpacity }, children: "Drag the page edges or use the buttons below to interact." }), _jsx(Paper, { elevation: 3, sx: { position: 'fixed', bottom: 24, left: '50%', transform: 'translateX(-50%)', display: 'flex', alignItems: 'center', p: 1.5, px: 3, borderRadius: 2, background: 'rgba(255,255,255,0.95)', backdropFilter: 'blur(8px)' }, children: _jsx(Typography, { sx: { color: 'text.primary', fontFamily: 'Georgia, serif', fontSize: 14, fontStyle: 'italic', minWidth: 100, textAlign: 'center' }, children: pageLabel }) })] }));
}
